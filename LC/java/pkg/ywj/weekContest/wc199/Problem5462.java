...
public class Problem5462 {
    ...
    private int[] dfs(String s, int k, int st, int size, int[][] dp) {
        if (st == size) return new int[] {0, 0};

        int[] r = new int[] {dp[st][k], 1};
        if (r[0] != -1) return r;

        // case1: 删除当前位置字符
        if (k > 0) {
            int[] x = dfs(s, k - 1, st + 1, size, dp);
            r[0] = x[0];
        }

        // case2: 保留当前字符，并枚举该字符连续的长度（所以遇到不同字符必须删除，才能维护其连续性）
        int c = r[1]; // 记录当前字符的连续个数
        for (int i = st + 1; i <= size; i++) {
            int judge = c >= 100 ? 3 : c >=10 ? 2 : c > 1 ? 1: 0;
            int t = dfs(s, k, i, size, dp)[0] + 1 + judge;

            if (r[0] == -1 || r[0] > t) r[0] = t;
            if (i == size) break;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(su.getLengthOfOptimalCompression("aaabcccd", 2)); // 4
